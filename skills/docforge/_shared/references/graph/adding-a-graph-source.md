...
Docforge reads a **code graph** (required) and, where available, a **flow graph** through a small registry of *sources*. Three ship today — Understand-Anything, GitNexus, and CodeGraph — and adding another (graphify, …) touches exactly three things. Nothing in `precheck_graph.{py,js}`, `read_graph.{py,js}`, or any instruction file changes (scripts and README: [`../../runtime/graph/README.md`](../../runtime/graph/README.md)).

## Naming convention

Everything specific to your source carries the `graph_source_<name>` prefix: the scripts `runtime/cli/python/graph_source_<name>.py`/`.js`, an optional offline reader `runtime/cli/python/graph_source_<name>_reader.py`/`.js` if it is DB-backed, and the reference `references/graph-source-<name>.md`. Source-agnostic pieces (`graph_source_registry`, `read_graph`, `precheck_graph`, `graph_storage`) never carry a source name. Each source-specific file opens by stating which source and need it serves.

## The `SOURCE` descriptor

Write `runtime/cli/python/graph_source_<name>.py` and `runtime/cli/python/graph_source_<name>.js` (both — every docforge script is a dual mirror). Each exposes a `SOURCE` descriptor:
...
- **`setup_hint(repo, gap)`** returns the lines `precheck_graph.{py,js}` prints when that capability is missing. `gap` is `"code_graph"` or `"flow_graph"`. Indented command lines (four spaces) are printed as-is; other lines get a two-space prefix.
- **`entry_points(repo)`** *(optional)* returns ranked flow-derivation seeds — `[{"id", "name", "kind", "path", "rank"}]`, highest `rank` first — read from your source's own entry-point signal (framework routes, exported-but-uncalled functions, entry-point tags…), **never a full scan**. `derive_flow_graph.{py,js}` uses it to build an entry-point-first, main-flow-first context instead of dumping the whole graph. Omit it and a `json` source falls back to the flat dump; a `db`/`mcp` source without it routes to its native interface (`mcp-explore` / `native-interface`). Provide it when your source carries cheap entry-point signal — it is what makes flow derivation focused rather than a soup. See `references/graph/flow-derivation.md`.

### Capabilities

- `code_graph` — structure and call/import relationships. Universal precondition. A source that provides only this is fully useful; docforge derives a flow graph from it when needed (`references/graph/flow-derivation.md`).
...
Add one import + one list entry, in priority order, to **both** `runtime/cli/python/graph_source_registry.py` and `runtime/cli/js/graph_source_registry.js`:
...
Write `references/graph-source-<name>.md` (the source-specific binding — what it stores, how to detect, how to read, how to build/refresh), and add one column to the capability table in `references/graph/graph-sources.md`. That is the only instruction-prose change: everything else already speaks in capabilities and resolves through that table.

...
- Adding a new public command → implement it in both Python and JS under the
  matching subsystem, then add a launcher pair in `cli/python/` and
  `cli/js/`.
...
package-qualified path (Python) or a relative `require()` (JS) — never the
reverse. No subsystem module imports from `cli/`.

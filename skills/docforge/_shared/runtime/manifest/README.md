...
| `set-graph [--provider <id>] [--force]` | manifest | locks/self-heals `manifest["graph"]`; auto-picks the highest-priority ready source when `--provider` is omitted; switching an already-locked provider requires `--force`. The stored `flow` describes the **chosen provider only** (`flow_capability_of`), never repo-wide flow readiness — a CodeGraph lock records `derived`/`none` even beside a ready `.ua/domain-graph.json` |

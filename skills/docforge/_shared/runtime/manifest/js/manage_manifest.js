...
function foldCompactGroups(catalog, selected, audiences) {
  ...
  for (const groupId of [...groups.keys()].sort()) {
    ...
    if (!definition || (definition.selection || {}).mode !== "compact") {
      kept.push(...members.map(([, doc]) => doc).sort((a, b) => a.id.localeCompare(b.id)));
      continue;
    }
    members.sort((a, b) => a[0] - b[0] || a[1].id.localeCompare(b[1].id));
...
function parseArgs(argv) {
  ...
  const knownCommands = new Set(["init", "add", "set", "presentation", "audit", "status", "set-graph", "reconcile", "finish", "set-storage", "unmanaged", "retire"]);
  ...
  const allowed = {
    init: new Set(["repo", "tier", "scale-class", "layout", "shape", "platform", "framework", "concern", "audience", "overlay", "name", "force", "graph-provider", "storage"]),
...
function resolveScale(repo, scaleClass, layout) {
...
function cmdInit(args) {
  ...
  const projectScale = resolveScale(args.repo, args.scale_class, args.layout);
...
function cmdReconcile(args) {
  ...
  if (args.layout && args.layout !== currentScale.layout) {
    const scaleRecord = { ...currentScale };
...
function usage() {
  console.log("usage: manage_manifest.js init --repo <path> --tier <spine|diligence|portfolio> [--scale-class <small|medium|large>] [--layout <compact|standard>] [--shape <id>] [--platform <id>] [--framework <id>] [--concern <id>] [--audience <id>] [--storage <json|markdown>] [--graph-provider <id>] | add --repo <path> --type <type> --id <id> --path <path> [--evidence <path:...|graph:...|user-confirmed:...>] | set --repo <path> --id <id> --status <status> | presentation --repo <path> --id <id> [--primary-audience <id>] [--code <mode>] [--related-docs <mode>] [--repository-paths <mode>] [--reset] | audit --repo <path> --id <id> --mode <cold-pass> --verdict <PASS|FAIL> --report <path> | status --repo <path> | set-graph --repo <path> [--provider <id>] [--force] | set-storage --repo <path> --storage <json|markdown> [--dry-run] | unmanaged --repo <path> --action <list|add|remove|archive> [--path <rel>] [--dry-run] | retire --repo <path> --doc <id> [--doc <id> ...] --mode <obsolete|delete> [--dry-run] | finish --repo <path> [--keep-tmp]");
}
function main() {
  let args;
  try {
    ...
    return { init: cmdInit, add: cmdAdd, set: cmdSet, presentation: cmdPresentation, audit: cmdAudit, status: cmdStatus, "set-graph": cmdSetGraph, reconcile: cmdReconcile, finish: cmdFinish, "set-storage": cmdSetStorage, unmanaged: cmdUnmanaged, retire: cmdRetire }[args.command](args);
  } catch (error) {
    usage();
    return fail(error.message, 2);
  }

 ...
 * provider fit documented in references/graph/graph-sources.md.
 *
  ...
  if (ready.length > 1) {
    console.log(
      `  ${ready.length} sources are ready — ask the user which should be ` +
        "primary; select by the evidence need in references/graph/graph-sources.md."
    );
  }
  return true;
}
...
function printFlowRemediation(repo) {
  ...
  console.log(
    "  (a) Derive a provisional flow graph from the code graph " +
      "(see references/graph/flow-derivation.md):"
  );
  console.log("    node runtime/cli/js/derive_flow_graph.js prepare --repo <path>");
  console.log("    # dispatch the Docforge flow analyzer on the emitted context, then");
  console.log("    node runtime/cli/js/derive_flow_graph.js write --repo <path> --analysis <analysis.json>");

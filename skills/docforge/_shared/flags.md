...
| `--auto-accept` | Display plans/trees/results, then continue without routine conversational pauses; never authorizes provider installation, graph build/refresh, manifest initialization, root `README.md` migration choices, file archive/deletion (including `unmanaged archive` moves and `retire` retirement moves or deletes), or other side effects (see [`rules.md`](rules.md)) |
| `--no-dashboard` | Skip the automatic dashboard build/serve at run completion ([`workflows/validation.md`](workflows/validation.md) §7) — suppresses both the forced serve and, in compact layout, the dashboard offer. The run still validates and completes; render later with `/docforge-dashboard` (or the internal `dashboard start`). |

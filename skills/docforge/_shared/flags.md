...
| `--auto-accept` | Display plans/trees/results, then continue without routine conversational pauses; never authorizes provider installation, graph build/refresh, manifest initialization, root `README.md` migration choices, file archive/deletion (including `unmanaged archive` moves), or other side effects (see [`rules.md`](rules.md)) |

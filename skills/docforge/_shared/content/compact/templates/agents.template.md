...
| View | Answers | Owning human document |
|---|---|---|
| {{Section name}} | {{the question this view answers}} | {{relative link}} |
...
{{What belongs in the coding-agent section, and what is owned by a human
document instead. Link `AGENTS.md` as the kernel — entry points, verified
commands, and precedence live there and are never restated here. Link every
selected document under `docs/agents/` that this file does not merge, plus
`CLAUDE.md`, `CLAUDE.local.md`, and `.claude/settings.json` when selected.}}

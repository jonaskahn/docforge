...
def parse_frontmatter(path: Path) -> list[dict] | None:
...
def sync_provenance(manifest: dict, repo: Path) -> int:
    updated = 0
    for doc in manifest["documents"]:
        sections = parse_frontmatter(repo / doc["path"])
        if sections is None:
            continue
        doc.setdefault("provenance", {})["sections"] = sections
        updated += 1
    return updated


def check(manifest: dict, repo: Path, section_filter: str | None) -> tuple[list[dict], bool]:
    results: list[dict] = []
    clean = True
    for doc in manifest["documents"]:
        if doc.get("status") not in WRITTEN:
            continue
        sections = doc.get("provenance", {}).get("sections", [])
        matching = [
        ...
        for section in matching:
            for source in section.get("sources", []):
                ...
                elif current != source.get("git_blob"):
                    stale.append({
                        "doc": doc["path"], "status": "PARTIAL",
...
def main() -> int:
    ...
    if args.sync_provenance:
        synchronized = sync_provenance(manifest, repo)
        args.manifest.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    results, clean = check(manifest, repo, args.section)
    if args.json:

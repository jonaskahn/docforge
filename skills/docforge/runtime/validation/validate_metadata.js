...
function validate() {
  ...
  for (const [name, tokens] of Object.entries(PUBLIC_CONTRACTS)) {
    for (const suffix of ["py", "js"]) {
      // CLI flags live in the runtime implementation; the scripts/ launcher
      // is a thin `require(...)` re-export with no flag text.
      const source = findFile(runtimeRoot, `${name}.${suffix}`) || path.join(scripts, `${name}.${suffix}`);
  ...
  function collectSourceFiles(dir, out) {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      else if (/\.(py|js)$/.test(entry.name)) out.push(full);
    }
  }
  const scannable = [];
  for (const name of names.filter((name) => /\.(py|js)$/.test(name))) {
    scannable.push(path.join(scripts, name));
  }

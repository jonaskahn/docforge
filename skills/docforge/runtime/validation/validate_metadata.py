...
def validate() -> list[str]:
    ...
    py_names = {path.stem for path in scripts.glob("*.py")} - peerless
    js_names = {path.stem for path in scripts.glob("*.js")} - peerless
    ...
    for name, tokens in PUBLIC_CONTRACTS.items():
        for suffix in ("py", "js"):
            # CLI flags live in the runtime implementation; the scripts/
            # launcher is a thin `import *` re-export with no flag text.
            matches = list(runtime_root.rglob(f"{name}.{suffix}"))
            source = matches[0] if matches else scripts / f"{name}.{suffix}"
    ...
    scannable = list(scripts.glob("*.[pj][ys]")) + list(runtime_root.rglob("*.[pj][ys]"))

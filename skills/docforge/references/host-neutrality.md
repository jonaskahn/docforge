...
**Links.** Relative paths (`../architecture/overview.md`) work when browsing the forge, in most editors, and in generated static sites. Absolute URLs to the current host break on migration and on forks. Never link to a line number in another file; link to the file and name the symbol.

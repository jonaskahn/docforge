...
- `flow_graph` — business-flow/domain graph. Optional. Provide it only if the source genuinely emits flows (understand-anything and GitNexus both do; CodeGraph doesn't, so derivation is what covers flow docs when it's the only ready source).

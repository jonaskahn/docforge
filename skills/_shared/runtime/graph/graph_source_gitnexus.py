...
is it stale against the current git HEAD? See references/graph/graph-source-gitnexus.md
for the read recipes and the capability dispatch in references/graph/graph-sources.md.
...
def setup_hint(repo: Path, gap: str) -> list[str]:
    ...
    return [
        ...
        "runtime/cli/python/graph_source_gitnexus_reader.py (references/graph/graph-source-gitnexus.md).",
    ]
...
def run_detect(args: argparse.Namespace) -> int:
    ...
    if result["index"] or result["code_graph"]:
        ...
        print("  Read via the gitnexus MCP, or offline with "
              "runtime/cli/python/graph_source_gitnexus_reader.py.")
        return 0
